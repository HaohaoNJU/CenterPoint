...
# predicted label to classname 
LABEL2CLASSNAME =  {0 : "Car", 1 : "Pedestrian",2:"Cyclist"}

ROOT_PATH = "/home/wanghao/Desktop/projects/CP_TRT/github/CenterPointTensorRT/results/fp/"
...
def get_dets_fname_cpp(frame_name, thr=THRESH):
    box9d = np.loadtxt(frame_name )
...
def kitti_waymo_coord_convert(bbox):
...
if  __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        box3d_preds, label_preds, scores = get_dets_fname_cpp(ROOT_PATH + "seq_%s_frame_%s.bin.txt"%(SEQ_IDX, frame))
        # box3d_preds, label_preds, scores, velocities = get_dets_fname_gt(ROOT_PATH + "seq_%s_frame_%s.pkl"%(SEQ_IDX, frame))
        box3d_preds[:,:7] = kitti_waymo_coord_convert(box3d_preds[:,:7]) # actually, waymo to kitti coord ! 

        if TRACK : 
            # [x,y,z,dx,dy,dz,r,score,class
            # box3d_preds = kitti_waymo_coord_convert(box3d_preds)
            ...
            box3d_preds = mot_tracker.update(box3d_preds, velocities= None) #velocities[:,:2])
