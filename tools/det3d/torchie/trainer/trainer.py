...
def parse_second_losses(losses):
    ...
    for loss_name, loss_value in losses.items():
        if loss_name == "loc_loss_elem":
            log_vars[loss_name] = [[i.detach().item() for i in j] for j in loss_value]
        else:
            log_vars[loss_name] = [i.detach().item() for i in loss_value]
...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        tb_path = os.path.join(work_dir,'tensorboard')
    ...
    def train(self, data_loader, epoch, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            # TODO : Float32 Or Apex Mix Precision
            # if self.mix_prec:
            #     self.amp_after_train_iter(self.outputs,scaler)
            # else:
            self.call_hook("after_train_iter")
            if self.tb_writer is not None:
                param_groups = self.optimizer.param_groups
                for i,param_group in enumerate(param_groups):
                    self.tb_writer.add_scalar('lr_group{}'.format(i),param_group['lr'].item(),self._iter )
                self.tb_writer.add_scalar("loss",outputs['loss'].detach().item(), self._iter)
            #=================================================================================================================================>
    ...
    def resume(self, checkpoint, resume_optimizer=True, map_location="default"):
        ...
        if "optimizer" in checkpoint and resume_optimizer:
            ...
            self.logger.info("resumed epoch %d, iter %d, with optimizer loaded", self.epoch, self.iter)
        else:
            self.logger.info("resumed epoch %d, iter %d, without loading optimizer ", self.epoch, self.iter)
    ...
    def register_training_hooks(
        ...
        if self.mix_prec:
            print("Mix prec on the go")
            self.register_hook(self.build_hook(optimizer_config, AmpOptimizerHook))
        else:
