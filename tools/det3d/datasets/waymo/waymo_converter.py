...
pattern2name = pkl.load(open("/mnt/data/waymo_opensets/train/tfPattern2sceneFrameTrain.pkl",'rb'))
...
def convert(idx):
    ...
    prefix = pattern2name[name[8:-9-16-3]]
    ##############################################################################
    for frame_id, data in enumerate(dataset):
        ...
        with open(os.path.join(LIDAR_PATH, prefix.format(frame_id)), 'wb') as f:
            pickle.dump(decoded_frame, f)
        with open(os.path.join(ANNO_PATH, prefix.format(frame_id)), 'wb') as f:
