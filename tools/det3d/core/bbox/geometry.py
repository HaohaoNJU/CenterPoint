    ...
    for i in range(num_points):
        for j in range(num_polygons):
            for k in range(max_num_surfaces):
                # TODO ： 锁定任意point与任意一个box的任意一个surface
                # 计算这个surface与这个point之间的点积与对应的d值作为sign
                # 其实计算的就是这个surface第一个点到这个点的向量与这个surface的法向量之间的点积，
                # 如果点积大于0，说明这个点与这个法向量夹角不超过90度，由于法向量指向box外侧，说明这个点不在这个surface内侧
                # 最后返回的值指向所有box里面的有效点数量。
                if k > num_surfaces[j]:
                    break
                sign = (
...
def points_count_convex_polygon_3d_jit(points, polygon_surfaces, num_surfaces=None):
    ...
    # TODO ： d就是每个surface的第一个point与这个surface的法向量之间的element-wise 乘积
